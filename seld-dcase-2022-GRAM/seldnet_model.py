...
class GRAM(torch.nn.Module):
    def __init__(self, weights, in_feat_shape, out_shape, params):
        super().__init__()
        ...
        self.model.train()
        # Downsample the output hz from 80ms per time-step to 100ms per time step.
        # Goes from 2s -> 25 to 2s -> 20.
        self.conv_downsample = nn.Conv1d(in_channels = self.model.encoder_embedding_dim, 
                                         out_channels = self.model.encoder_embedding_dim,
                                         kernel_size = 6,
                                         stride = 1,
                                         bias = False)
        ...
        self.fnn_list.append(
            torch.nn.Linear(self.model.encoder_embedding_dim, out_shape[-1], bias=True)
        )
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "B Seq (Embedding F) -> B Seq Embedding F", F = self.model.grid_size[0])
        # Take the mean over the frequency dimension
        x = x.mean(dim = -1)
        x = rearrange(x, "B Seq Embedding -> B Embedding Seq")
        x = self.conv_downsample(x)
        x = rearrange(x, "B Embedding ReducedSeq -> B ReducedSeq Embedding")
