...
import os
import numpy as np
import cls_feature_class_gram as cls_feature_class
    ...
    def get_data_sizes(self):
        ...
        if self._is_eval:
            ...
            if self._multi_accdoa is True:
                # where C is the number of target classes and N the maximum assumed number of co-occuring events in the recordings, which is 3!
    ...
    def generate(self):
        ...
        if self._is_eval:
            for i in range(self._nb_total_batches):
                ...
                feat = rearrange(feat, 'T (C F) -> T C F', F = self._nb_mel_bins, C = self._nb_ch)
            ...
            for i in range(self._nb_total_batches):
                ...
                for j in range(self._feature_batch_seq_len):
                    feat[j, :] = self._circ_buf_feat.popleft()
                feat = rearrange(feat, 'T (C F) -> T C F', C = self._nb_ch, F = self._nb_mel_bins)
